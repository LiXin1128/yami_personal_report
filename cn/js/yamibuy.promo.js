	...
	(function($, undefined) {
		...
		$.yamibuy.user = user = {
			...
			signOut : function(){
				...
				$.yamibuy.common.doAjax('http://www.yamibuy.com/cn/service.php', {act: 'logout'}, function(response){
					if(response.status == 1){
			...
			displaySignIn : function (data) {
				if(user.isSigned() == false){
					$.yamibuy.common.doAjax('http://www.yamibuy.com/cn/service.php', 
							{act:'viewMiniLogin'}, 
							function(response){
					...
					},{crossDomain : true, showloading : true, method : "POST", dataType : "JSON"});
				}else{
					user._execute(data);
				}
			},
